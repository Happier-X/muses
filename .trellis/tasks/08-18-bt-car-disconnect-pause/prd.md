...
- [ ] 本地源播放中拔出蓝牙耳机 → 通知仍在、播放暂停、按钮可恢复。
- [ ] WebDAV 源播放中拔出蓝牙耳机 → 暂停（同上）。
- [ ] CarWith 连接播放中拔线/断开 → 暂停。
- [ ] 暂停状态拔出设备 → 无状态抖动（仍暂停）。
- [ ] 拔出后重新连接设备 → **不**自动恢复播放（保持暂停，用户手动恢复）。
- [ ] 播放中插入设备、切换设备（蓝牙→扬声器切换）不误暂停。
- [ ] 暂停/恢复/切歌/seek/自动切歌（含 CarWith 场景，配合任务 08-18-carwith-bg-ctrl-fix）不回归。
