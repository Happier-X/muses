...
1. [ ] `07-22-scan-batch-persist`
   - 扫描纯内存 upsert，成功后单次 saveSongs
   - 进度回调节流，最终态立即
   - 验证失败原子性和更新广播次数
2. [ ] `07-22-queue-linear-resolve`
   - 单次曲库加载 + songId Map
...
3. [ ] `07-22-virtualize-long-lists`
   - QueuePage、PlaylistDetailPage 优先
   - Albums/Artists 按宽屏布局兼容方案处理
4. [ ] `07-22-hidden-player-render-budget`
   - 保活前提下隐藏绘制
   - 冻结 AMLL 时间输入
   - 真机观察重开闪烁和 CPU/GPU
...
5. [ ] `07-22-optimize-amll-index`
   - 标题候选索引
   - JSONL 分片解析并让出事件循环
6. [ ] `07-22-runtime-cache-lifecycle`
   - bounded cache / LRU
