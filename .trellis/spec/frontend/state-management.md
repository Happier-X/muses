...
- 本地标签扫描后若当前曲仍无安全 `coverUri`，可异步在线匹配封面（iTunes → kw → mg → kg → tx → wy）：下载到 app `cache/covers` 后 `upsertSong` 写回安全 `file://` URI，并 `syncDisplayStateFromSong` + 媒体会话封面；已有封面不覆盖；失败静默；token 防串曲。
