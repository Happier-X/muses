...
- A successful source scan is an atomic batch that must **rebase before commit**: during the discovery loop only collect each file’s `UpsertSongInput` (no in-memory accumulation against a snapshot); after the loop, `const latest = loadSongs()` and replay every collected input via `upsertSong(input, latest, { persist: false })`, then reconcile with `reconcileSourceSongs(sourceId, keepPaths, replayed, { persist: false })`, then call `saveSongs` at most once when the final library changed. Rationale: other writers (player lazy metadata scan writing embedded lyrics, user edits) may persist while a long WebDAV scan is still running; committing a stale baseline array would silently wipe their writes (08-22-fix-scan-overwrite-lazy-lyrics). Same-song conflicts on both sides are resolved by `upsertSong`’s non-destructive field merge (`?? previous`). A fatal error before commit leaves storage unchanged. Per-file upsert failures remain degraded batch entries: increment `failed`, keep the discovered path in `keepPaths`, and continue so an existing record for that path is not deleted as stale.
...
- Scanner tests assert the lost-update regression: an external `upsertSong` (e.g. player lazy scan writing embedded lyrics) that lands between baseline capture and commit survives the final `saveSongs`; same-song dual writes keep both sides’ non-conflicting fields.
...
- 本地来源化（08-21-local-first-meta-lyrics）：播放器**完全禁用**自动在线匹配——切歌/播放/预取不再请求在线歌词、在线封面、在线文本元信息；在线补全能力统一收敛到刮削页（用户主动操作）。
- 歌词展示仅来自本地：内嵌歌词 / 同目录同名 `.lrc`（sidecar）；无本地词显示空态，文案引导去刮削页。切歌后直接取库内值：有词 `onlineLyricsStatus = 'ready'`，无词 `'miss'`，不再有 `'matching'`/`'error'`。
- 歌曲信息（title/artist/album/cover）展示仅来自内嵌 tag 或用户手动操作（编辑 sheet 手改 = manual、刮削页写回 = embedded/scrape）；不再有自动 cloud 补缺。`prefetchNextTrack` 仅保留 WebDAV 下一首音频完整预取，元信息三路预取已移除（`prefetchMetadata.ts` 已删除）。
- 存量清理迁移（一次性）：`loadSongs()` 首次调用执行 `muses:migration:local-first-v1` 打标迁移——清除 `lyricsSource === 'online'` 的歌词与 `metaSources.* === 'cloud'` 字段；`userEditedFields` 保护字段跳过；title 回退文件名兑底标题。`'online'`/`'cloud'` 作为历史遗留值保留在类型联合中仅供校验/迁移/可疑判定消费，新代码禁止写入。
- 来源标记：`LyricsSource = 'embedded' | 'sidecar' | 'scrape' | 'online'(遗留)`；`FieldSource = 'embedded' | 'scrape' | 'manual' | 'cloud'(遗留)`。刮削页写回文件成功标 `embedded`，失败标 `scrape`（值得重刮）。
