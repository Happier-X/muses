...
- **侧边栏视觉契约（08-16-sidebar-card-style 定案，MuMu 12.2.0 椒盐实机对比，二版迭代最终形态）**：移动端抽屉（`.tabs-layout__drawer`，推屏轨道内 50vw 槽位）= **透明槽位** + 内部 `.tabs-layout__panel` 呈现**椒盐式悬浮灰色卡片**：`margin: 0 12px 0 18px`（左空隙 18dp、右空隙 12dp，椒盐实测卡片 x48..552 即 18+504+12）、背景 `--m-surface-1`（浅 #f9f9f9 / 深 #262626，纯色无玻璃）、`border-radius: 24px`、`border: 1px solid var(--m-hairline)`、阴影 `0 8px 24px rgba(0,0,0,0.08)`（深色 0.35）。**无 header 区**（用户第二轮明确不要顶部按钮；椒盐虽有 退出/用户界面/音频效果 三按钮，用户主观弃用，禁止再加回）。卡片内：主菜单（`.tabs-layout__nav--primary`，padding-top 8px）+ 次菜单（`.tabs-layout__nav--secondary`，`margin-top: 9px + padding-top: 9px + border-top 1px hairline`）。菜单项（`.tabs-layout__nav-link`）：行高 **64px**、字号 16px、**drawer 内左侧无 padding（`&__drawer &__nav-link { padding-left: 0 }`）**、图标区 `.tabs-layout__nav-icon-shell` 固定 **60px** 宽（图标 24px 居中）→ 文字起点 = 18(空隙)+60 = **78px**（椒盐实测 '歌曲' text x204px@2.625 = 77.7dp ✓）。**激活态无背景、图标也不变色**（用户第二轮明确图标不要蓝色）：图标恒 `--m-text-2`（#8c8c8c），激活仅 `font-weight: 600`；按压瞬态 `:active { background-color: rgba(var(--m-primary-rgb), 0.08) }`（非常驻）。**禁止** :active 以外的常驻蓝底/蓝图标。图标恒为 lucide 线性（用户确认不改彩色方块）。侧边栏顶部 padding = `var(--m-safe-area-top)`（紧贴状态栏下沿），底部保留 `var(--m-spacing-sub) + var(--m-safe-area-bottom)`。**平板 aside 不卡片化**（常驻 260px 侧栏保持 surface-1 + border-right 现状）。
