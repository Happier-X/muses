...
- **k-glass 仅用于临时浮层**（toast / dialog 等瞬时 UI），不再用于常驻条（MiniPlayer / 随机播放吸顶条已改双层灰玻璃）。
...
- **沉浸页按下态必须统一**（`08-04-player-immersive-btn-press-unify`，`08-15-player-icon-button-unify` 起按椒盐实测更新）：播放页 9 个纯图标按钮（主控 3×lg、mode-bar 4×md、歌词翻译 FAB md + 播放 FAB lg）统一用 **`m-icon-button`**（透明底 + `color: inherit`），按下反馈 = 组件自带 **currentColor 半透明涟漪**（`:active` 涟漪 opacity 0.1，深色模式 0.12；`rgba(255,255,255,*)` 白系按钮在深色播放页上即白色半透明涟漪 + `while-tap` scale 0.88 缩放）；调用方只通过 scoped class 给 `color: rgba(255,255,255,*)` 白系色（**禁止 `!important`**，MIconButton `color: inherit` 无更高优先级冲突，图标 `currentColor` 跟随）。涟漪 `inset: 0` 圆形底 + 图标 `z-index: 1` 分层，motion 内联 transform 与 ripple CSS transform 分属按钮/子元素，无冲突。**禁止**回退 `m-button`（`clear` 变体）或 PlayerPage scoped 手写 `::after` 涟漪 / `:active` 背景覆盖；编辑 sheet 等文字按钮继续用 `m-button`。
