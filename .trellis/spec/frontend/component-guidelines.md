...
- **列表项左侧图标容器规格（08-23-settings-page-style）**：设置类页面的 `MListItem` 用 `#media` 插槽放图标容器：36×36px 圆角方形（`--m-radius-sm`）+ `rgba(var(--m-primary-rgb), 0.12)` 底 + `var(--m-primary)` 图标色 + 20px lucide 线条图标，`flex-shrink: 0`、右距 `var(--m-spacing-sub)`；主色透明底必须用 `--m-primary-rgb` 变量，明暗主题自动跟随无需 `:global(.dark ...)` 覆盖。装饰性图标容器加 `aria-hidden="true"`。参考实现 `src/views/SettingsPage.vue` 的 `.settings-page__item-icon`。
...
- **底部几何契约**：移动端与平板均无底部 Tabbar；MiniPlayer 悬浮胶囊高 64px、固定在 `bottom: calc(var(--m-safe-area-bottom, 0px) + 8px)`（胶囊底部留 8px 悬浮空隙，08-14 胶囊化）。MiniPlayer 无歌曲时仍显示空状态，内容止位始终为 `calc(72px + var(--m-safe-area-bottom, 0px))`（64px 高度 + 8px 悬浮空隙 + 底部安全区；08-16 修复——此前只算 64px，列表滚到底时最后一项底部 8px 被胶囊盖住）。改变 MiniPlayer 高度/悬浮空隙或安全区算法时必须同步检查 `--m-content-pb`、`--m-content-pb-md` 与 MiniPlayer 定位，禁止留下空带或重复避让。**`--m-content-pb` / `--m-content-pb-md` 必须定义在 `.m-app` 内（08-18-fix-content-pb-safe-area-freeze）**：定义在 `:root` / `html.muses-mini-visible` 时，Chromium 计算根元素的 custom property 会立即展开其 `var(--m-safe-area-bottom)`（:root 默认 0px），把 `calc(72px + 0px)` 冻结为常量继承，`.m-app` 桥接出的真实安全区（手势导航时 = 手势条高度）无法再影响列表止位，列表滚到底时末项被 MiniPlayer 盖住约一整个手势条高度（headless 实测复现：注入 `--safe-area-inset-bottom: 24px` 时 padding 恒 72px 而 MiniPlayer 上移到 bottom 32px，重叠 24px；将定义移入 `.m-app` 后 padding 变 96px、重叠归零）。**禁止**把内容止位 token 移回 `:root` 或用 `html.muses-mini-visible` 重复定义。
...
- **安全区处理**：Capacitor 8 注入的 `--safe-area-inset-*` 与 env() 兜底由 `index.scss` 的 `.m-app` 提供 `--m-safe-area-*` 桥接，所有组件均使用 `--m-safe-area-*` 避让。**含安全区计算的内容避让 token 必须定义在桥接作用域 `.m-app`**（`--m-navbar-pt`、`--m-content-pb` 均为该模式；08-18-fix-content-pb-safe-area-freeze 明确禁止把这类 token 定义在 `:root`——Chromium 会在根元素上立即展开 `var()` 依赖并冻结为常量，见「底部几何契约」）。**navbar 顶部避让统一 token（08-15-navbar-status-bar-spacing 起，08-15 末修正）**：`.m-app` 定义 `--m-navbar-pt: max(16px, var(--m-safe-area-top, 0px))`（**紧贴状态栏下沿**——椒盐 MuMu 1080x1920 像素实测：状态栏 24dp、navbar 行 56dp 无缝衔接、间距 0；此前 `+8px` 呼吸间距与 SongsPage `+6px`（22px“椒盐基准”）均源于把“图标在行内的位置”误判为行间距，已废除；浏览器 safe-area=0 时回落 16px）。`MNavbar` 的 `padding-top` 与全项目内容避让公式（如 `calc(var(--m-navbar-pt, 16px) + 44px)`）一律消费此 token，**禁止**再新写 `max(Xpx, var(--m-safe-area-top, 0px))` 裸公式或任何 `+8px`/`+6px` 差值；自 `08-15-toolbar-navbar-same-glass` 起 navbar 含 48px subnavbar（工具条/搜索栏），内容避让公式为 `calc(var(--m-navbar-pt, 16px) + 44px + 48px)`。
- **弹层 z-index 阶梯**：移动端推屏导航位于普通页面层级；MiniPlayer (1000) < MPopup (1100) < MSheet/MDialog/MActions (1200) < MToast (1300)。**防坑（08-15-toast-not-showing）**：TabsPage `.tabs-layout__track`（motion 推屏轨道）恒有 `transform: translateX(-50vw)` + `will-change: transform`，会同时成为页面内 `position: fixed` 元素的包含块并创建 z≈0 的层叠上下文——页面内直接渲染的 fixed 浮层会整体低于根层叠上下文的 MiniPlayer z-1000 而被盖住；且 fixed 元素改为相对 track 定位——track 宽 100vw（50vw 抽屉 + 50vw 主区）且左移 50vw，其 `left: 50%` 中点落在屏幕最左缘，对话框会整体偏左（08-21-fix-overlay-fixed-containing-block）。**所有页面内浮层必须 `<Teleport to="body">` 渲染**（MToast/MActions/MDialog/MSheet/MPopup 五个组件均已遵此），并注意 teleport 后脱离 `.m-app`，`--m-safe-area-*` 桥接失效，bottom/padding 需两级兜底到 `--safe-area-inset-*` 与 `env()`（见下方安全区条目）。**遗留（08-21 同任务发现，未修）**：SongsPage `__multibar` 多选底部操作条仍渲染在 track 内且消费 `--m-safe-area-bottom`，存在同类偏移（内容中心偏左约 25vw）；因 jump-fab/index-bar 依赖 track 几何、multibar 依赖安全区桥接，不能简单 teleport，需单独设计修复。
...
- **平板（横屏 ≥768px 且宽>高，08-17-tablet-immersive-player 起 class 驱动）布局契约**：panels 恢复 flex row 左右并排（左信息面板 + 右歌词面板各 50%），左侧**不再展示三行歌词窗口**，**底部全宽控制条承担进度/控制/模式**（见 features-player.md）。**断点机制**：不再用 `@media (min-width: 768px)`（竖屏平板 800x1280 会落进 media query 缝隙且不与椒盐竖屏行为一致），改 JS `isTabletLayout = viewportWidth >= 768 && viewportHeight < viewportWidth` + 容器 class `.player-page--tablet`（scoped）/ `.player-overlay--tablet`（全局）。关键规则：
  - **`.player-page__panels` 的 `.player-page--tablet &` 覆盖 `width: 100% + flex: 1 1 auto + min-height: 0`（必须）**：scoped 基础 `width: 200%` 与全局 `.player-overlay--tablet .panels` 的 `width: auto` 同特异性（0,2,0），而组件 scoped（异步 chunk `<link>` appendChild 到 head 末尾）恒后注入覆盖全局——不重申则容器保持 200% 宽、右侧歌词面板被裁出视口。全局 `transform: none !important` 依赖 `!important` 才能压过 motion 内联 transform，由 `.player-overlay--tablet .panels` 承担。
  - **`.player-page__song-meta` 的 `.player-page--tablet &` 覆盖 `display: none`**（左侧无三行歌词；<768px 恢复 block，手机零回归）。
  - **`.player-page__info-controls` 包裹 progress/controls/mode-bar**（手机 `display: contents` 保持 info-inner flex 子项展开；平板 `display: none` 由底部条承担）。
  - **`.lyric-header` 裸类必须保留**（`class="lyric-header player-page__lyric-header"`）：右侧歌名/歌手在平板下由全局 `.player-overlay--tablet .lyric-header { display: none }` 隐藏（左栏 in-panel 头部承担，避免重复）；若改成纯 scoped 类，全局规则失配，平板右侧会重复显示顶部歌名。
  - **全局 index.scss 死代码类名**（08-14 椒盐复刻后不匹配当前 DOM，勿依赖）：`.cover-slot` / `.cover` / `.song-info` / `.lyric-play-toggle`；`.placeholder-cover` 仅匹配空状态图标。存活：`.panels`（transform/flex-direction）、`.panel`（flex:1 分栏）、`.info-panel`（padding）、`.info-panel-inner`（height/max-height；justify-content:center 与 gap:12 被 scoped flex-end/14 覆盖，以 scoped 为准）、`.lyric-player`（平板字号变量）。
...
- **侧边栏视觉契约（08-16-sidebar-segmented-cards 定案，三版迭代最终形态；MuMu 12.2.0 椒盐 s2 干净打开态线性扫描）**：移动端抽屉（`.tabs-layout__drawer`，推屏轨道内 50vw 槽位）= **透明槽位**，内部主/次菜单各为一张**独立圆角卡片**（`.tabs-layout__nav--primary` / `--secondary`，`&__drawer &__nav` 卡片样式）：`margin-right: 12px + margin-left: 18px`（左空隙 18dp、右 12dp，椒盐实测 x48..551）、背景 `--m-surface-1`（浅 #f9f9f9 / 深 #262626）、`border-radius: 16px`（椒盐圆角 ~15dp：y1130 左缘 x56 → y1148 x80 线性扫描）、`border: 1px solid var(--m-hairline)`、**无 box-shadow**（椒盐段间空隙实测纯 #f3f3f3 无投影；08-16 二版 24px 圆角 + 8px24px 阴影系臆造已撤）。主卡 padding-top/bottom 8px；次卡与主卡间 `margin-top: 18px`（椒盐段间 48px = 18dp）+ `border-top: none`（卡片自带描边）。**无 header 区**（用户第二轮明确不要顶部按钮，禁止加回）。菜单项（`.tabs-layout__nav-link`）：行高 **64px**、字号 16px、**drawer 内左侧无 padding（`&__drawer &__nav-link { padding-left: 0 }`）**、图标区 `.tabs-layout__nav-icon-shell` 固定 **60px** 宽（图标 24px 居中）→ 文字起点 = 18(空隙)+60 = **78px**（椒盐 '歌曲' x204px@2.625 = 77.7dp ✓）。**激活态无背景、图标也不变色**：图标恒 `--m-text-2`（#8c8c8c），激活项**无任何视觉区分**（08-16-sidebar-remove-active-bold：深黑加粗也已删除，与普通项同色同字重；仅保留 --active 类与 aria-current 语义）；**无任何按压背景效果**（08-16-sidebar-remove-active-feedback：`&:active` 蓝色按压背景已删除——此前仅非激活项生效造成不一致，用户要求全部取消）。**禁止** 任何常驻/按压蓝底或蓝图标。图标恒为 lucide 线性。侧边栏顶部 padding = `calc(var(--m-navbar-pt, 16px) + var(--m-spacing, 16px))`（08-16-sidebar-card-top-gap：像 navbar 一样顶部避让后**再留 16px 悬浮空隙**；浏览器 32px、真机状态栏下 16px），底部 `var(--m-spacing-sub) + var(--m-safe-area-bottom)`。**平板 aside 不卡片化**（常驻 260px 侧栏保持 surface-1 + border-right；aside 内分组仍用 9+9+hairline 的 `--nav--secondary` 样式）。
...
- **主题三态模式（useSystemDark，08-16-sidebar-salt-polish 新增；入口 08-16-sidebar-card-style 移除）**：`src/composables/useSystemDark.ts` 从纯跟随系统扩展为 `themeMode`（模块级 `Ref<'system'|'light'|'dark'>`）+ `cycleThemeMode()` 三态循环（system→light→dark），持久化 `localStorage['muses-theme']`；`useSystemDark()`（main.ts 调用）读存储并应用。system 时监听 prefers-color-scheme 切 `.dark`（原逻辑），light/dark 时解绑监听（手动模式优先）。**侧边栏无入口**（08-16 用户去掉了 header 三按钮，monitor/sun/moon 图标与 themeActionLabel 已随 TabsPage 一并移除；composable 保留供设置页后续提供主题选项），禁止在侧边栏加回按钮。
...
**Fix**：`.tabs-layout__nav-link` 统一 `outline: none`（与 MButton/MFab/MIconButton 等其他交互组件惯例一致），键盘无障碍由 `&:focus-visible { outline: 2px solid var(--m-primary); outline-offset: -2px; }` 保留——仅 Tab 键可见。**禁止**给抽屉链接单独加 `outline` 或移除 `focus()` 焦点陷阱逻辑（无障碍回退会失效）。**补充（08-22-fix-drawer-autofocus-ring）**：部分 WebView 对 JS focus 也会判定 `:focus-visible`，首次打开抽屉时首个菜单项出现主题蓝描边框——程序化聚焦统一走 `focusFirstDrawerLink()`：加 `--auto-focus` 标记类并以 `:focus-visible:not(&--auto-focus)` 排除描边，标记随 blur 移除、真实键盘导航焦点环不受影响；**禁止**删掉该标记机制或改回裸 `.focus()` 调用。
