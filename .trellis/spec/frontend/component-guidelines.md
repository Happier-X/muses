...
- **侧边栏视觉契约（08-16-sidebar-salt-polish，MuMu 12.2.0 椒盐实机对比定案）**：侧边栏（移动端 drawer + 平板 aside 共用 `.tabs-layout__panel` 结构）= 表面 `--m-surface-1` + `--m-hairline` 分隔，**无玻璃/无 blur**。结构自上而下：① header（`.tabs-layout__panel-header`，64px 高、底部 hairline；移动端三格均分按钮 ✕关闭 / 主题切换 / ⚙️设置，平板两格 主题切换 / ⚙️设置 —— 椒盐实测为 退出/用户界面/音频效果 三格均分）；② 主菜单（`.tabs-layout__nav--primary`，padding-top 8px）；③ 次菜单（`.tabs-layout__nav--secondary`，`margin-top: 9px + padding-top: 9px + border-top 1px hairline`（≈18dp 留白被线平分，椒盐实测主次组间空隙 y1103→1249 中 y1200 有 #e9e9e9 线））。菜单项（`.tabs-layout__nav-link`）：行高 **64px**、字号 16px、`padding: 0 var(--m-spacing)`、图标区 `.tabs-layout__nav-icon-shell` 固定 **60px** 宽（24px 图标垂直居中）→ 文字起点 ≈ 16+60 = **76px**（椒盐实测 '歌曲' text x204px@2.625 = 77dp，1px 差忽略）。激活态 **无背景高亮**（椒盐激活项文字同 #1E1715，无蓝底）：仅 `font-weight: 600` + 图标 `--m-primary`；按压瞬态 `:active { background-color: rgba(var(--m-primary-rgb), 0.08) }`（非常驻）。**禁止**再给 nav-link 加常驻蓝底 `rgba(primary, 0.12)`（08-16 对齐椒盐已去除）。图标恒为 lucide 线性（用户确认不改彩色方块）。侧边栏顶部 padding = `var(--m-safe-area-top)`（紧贴状态栏下沿，椒盐 header y100 起同理），底部保留 `var(--m-spacing-sub) + var(--m-safe-area-bottom)`。
...
- **主题三态模式（useSystemDark，08-16-sidebar-salt-polish 新增）**：`src/composables/useSystemDark.ts` 从纯跟随系统扩展为 `themeMode`（模块级 `Ref<'system'|'light'|'dark'>`）+ `cycleThemeMode()` 三态循环（system→light→dark），持久化 `localStorage['muses-theme']`；`useSystemDark()`（main.ts 调用）读存储并应用。system 时监听 prefers-color-scheme 切 `.dark`（原逻辑），light/dark 时解绑监听（手动模式优先）。侧边栏 header 主题按钮用 `monitor/sun/moon` 图标 + `themeActionLabel` 文案。禁止回退为只跟随系统（用户已在侧边栏入口要求手动切换）。
