...
- **MiniPlayer / SongsPage 跳转 FAB 实心胶囊契约（08-15-glass-solid-salt，MuMu 12.2.0 实测）**：两处均为**实心胶囊**，非玻璃、非 `--m-surface-1`——浅色纯白 `#fff` 实心（无半透明、无白边、无 blur），深色 `#1e1e1e`（比页面 `--m-surface` #202020 亮一档）。MiniPlayer 阴影 `0 4px 16px rgba(0,0,0,0.08)`（深色 0.35），FAB `0 2px 8px rgba(0,0,0,0.12)`；图标色必须用主题变量（MiniPlayer `var(--m-text)`、FAB `var(--m-text-2)`），禁止写死深棕 `#211715` / 灰 `#666`。深色覆盖在组件 scoped 样式**末尾**用 `:global(.dark) .xxx { background: #1e1e1e }`（编译后与 `.xxx[data-v-*]` 同特异性，靠后定义胜出，模式同 MToggle/MFab）。禁止加回 blur、白边或半透明底；不得改几何（尺寸/圆角/定位）。
...
- **k-glass 仅用于临时浮层**（toast / dialog 等瞬时 UI），不再用于常驻条（MiniPlayer / 随机播放吸顶条已改双层灰玻璃；08-15-glass-solid-salt 起 MiniPlayer 与 SongsPage FAB 进一步改为实心胶囊 `#fff`/`#1e1e1e`，见上方常驻导航表面条目）。
