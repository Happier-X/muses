...
- **沉浸页按下态必须统一**（`08-04-player-immersive-btn-press-unify`，`08-15-salt-player-btn-ripple` 起按椒盐实测更新）：播放器图标按钮用 `m-button`（`clear` + `rounded-full`）+ 白色 class 表达图标；**按下反馈 = 白色半透明涟漪（椒盐同款）**：PlayerPage scoped SCSS 对 `__side-btn`/`__play-btn`/`__mode-btn`/`__lyric-fab`/`__lyric-play-fab` 统一覆盖——`:active` 背景 `rgba(255,255,255,0.18)`（`!important` 压过 MButton clear 变体的主色蓝底 `rgba(var(--m-primary-rgb), 0.15)`）+ `::after` 白色径向渐变涟漪（`player-page-ripple` 0.42s scale 0→1.6 淡出，依赖 MButton 根元素既有 `position: relative; overflow: hidden` 裁剪）。涟漪为白色系，不改变按钮尺寸/图标/文字色与事件逻辑；仅作用于播放页，其他页面 clear 按钮仍为主色 15% 底。**禁止**依赖旧 `.h-button--ghost` 覆盖。
