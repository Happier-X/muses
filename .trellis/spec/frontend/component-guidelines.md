...
- **侧边栏视觉契约（08-16-sidebar-segmented-cards 定案，三版迭代最终形态；MuMu 12.2.0 椒盐 s2 干净打开态线性扫描）**：移动端抽屉（`.tabs-layout__drawer`，推屏轨道内 50vw 槽位）= **透明槽位**，内部主/次菜单各为一张**独立圆角卡片**（`.tabs-layout__nav--primary` / `--secondary`，`&__drawer &__nav` 卡片样式）：`margin-right: 12px + margin-left: 18px`（左空隙 18dp、右 12dp，椒盐实测 x48..551）、背景 `--m-surface-1`（浅 #f9f9f9 / 深 #262626）、`border-radius: 16px`（椒盐圆角 ~15dp：y1130 左缘 x56 → y1148 x80 线性扫描）、`border: 1px solid var(--m-hairline)`、**无 box-shadow**（椒盐段间空隙实测纯 #f3f3f3 无投影；08-16 二版 24px 圆角 + 8px24px 阴影系臆造已撤）。主卡 padding-top/bottom 8px；次卡与主卡间 `margin-top: 18px`（椒盐段间 48px = 18dp）+ `border-top: none`（卡片自带描边）。**无 header 区**（用户第二轮明确不要顶部按钮，禁止加回）。菜单项（`.tabs-layout__nav-link`）：行高 **64px**、字号 16px、**drawer 内左侧无 padding（`&__drawer &__nav-link { padding-left: 0 }`）**、图标区 `.tabs-layout__nav-icon-shell` 固定 **60px** 宽（图标 24px 居中）→ 文字起点 = 18(空隙)+60 = **78px**（椒盐 '歌曲' x204px@2.625 = 77.7dp ✓）。**激活态无背景、图标也不变色**：图标恒 `--m-text-2`（#8c8c8c），激活仅 `font-weight: 600`；**无任何按压背景效果**（08-16-sidebar-remove-active-feedback：`&:active` 蓝色按压背景已删除——此前仅非激活项生效造成不一致，用户要求全部取消）。**禁止** 任何常驻/按压蓝底或蓝图标。图标恒为 lucide 线性。侧边栏顶部 padding = `calc(var(--m-navbar-pt, 16px) + var(--m-spacing, 16px))`（08-16-sidebar-card-top-gap：像 navbar 一样顶部避让后**再留 16px 悬浮空隙**；浏览器 32px、真机状态栏下 16px），底部 `var(--m-spacing-sub) + var(--m-safe-area-bottom)`。**平板 aside 不卡片化**（常驻 260px 侧栏保持 surface-1 + border-right；aside 内分组仍用 9+9+hairline 的 `--nav--secondary` 样式）。
