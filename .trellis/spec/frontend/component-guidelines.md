...
- **`k-glass` 默认即完整 iOS 玻璃**（白 0.75 底 `bg-ios-light-glass` + `backdrop-blur-lg` 16px + iOS 内阴影），直接裸用（MiniPlayer / toast / 随机播放吸顶条）。**若要与 navbar 的灰渐变玻璃同构，必须双层结构**：blur 层（`backdrop-blur-[2px]` + `mask-b-from-50%`/`mask-b-to-100%`，无背景）+ bg 渐变层（`bg-gradient-to-b from-ios-light-surface to-transparent`，无 blur 无 mask），两层 absolute 分离（08-12 曾把 mask+渐变+blur 全叠在单个 k-glass 上 → mask 把 blur 与渐变下半一起裁掉 → 吸顶条下半无模糊、上半实心灰板）。
