...
- **常驻导航表面**：`MNavbar`、`MTabbar` 和列表吸顶操作条使用 `--m-surface-1` + `--m-hairline` 的干净表面，不使用 iOS 玻璃渐变、mask 或 `backdrop-filter`。`.m-glass-*` 兼容工具仍可保留，但不得用于常驻导航。
- **MiniPlayer / SongsPage 跳转 FAB 液态玻璃契约（08-15-glass-liquid-salt，MuMu 12.2.0 实测）**：椒盐「液态玻璃」= **实验室 Liquid Glass 开关**（仅 Android 13+，作用于迷你播放条；默认关 = 实心胶囊，开 = 半透明模糊）。本任务纠正 08-15-glass-solid-salt 的**实心误判**（当时实测的是 Liquid Glass 关闭的默认实心态，并非椒盐液态玻璃真容）。两处均为**半透明液态玻璃胶囊**，非 `--m-surface-1`：浅色 `rgba(255,255,255,0.85)`（比旧 0.72 更实）+ `blur(30px)`（必须 `-webkit-backdrop-filter` 前缀齐全）+ `border: 1px solid rgba(255,255,255,0.45)` 边缘高光，**底下列表内容模糊可见**；深色 `rgba(30,30,30,0.85)` + `border rgba(255,255,255,0.12)`。MiniPlayer 阴影 `0 4px 16px rgba(0,0,0,0.08)`（深色 0.35），FAB `0 2px 8px rgba(0,0,0,0.12)`（深浅一致）；图标色必须用主题变量（MiniPlayer `var(--m-text)`、FAB `var(--m-text-2)`），禁止写死深棕 `#211715` / 灰 `#666`。深色覆盖在组件 scoped 样式**末尾**用 `:global(.dark) .xxx { background: rgba(30,30,30,0.85); border-color: rgba(255,255,255,0.12) }`（浅色 border 用 shorthand，深色用 `border-color` 覆盖；编译后与 `.xxx[data-v-*]` 同特异性，靠后定义胜出，模式同 MToggle/MFab）。禁止回退实心 `#fff`/`#1e1e1e` 或去掉 blur/高光边；不得改几何（尺寸/圆角/定位）。
...
- **k-glass 仅用于临时浮层**（toast / dialog 等瞬时 UI），不再用于常驻条（MiniPlayer / 随机播放吸顶条已改双层灰玻璃；08-15-glass-liquid-salt 起 MiniPlayer 与 SongsPage FAB 为液态玻璃胶囊 `rgba(255,255,255,0.85)` + `blur(30px)`，见上方常驻导航表面条目）。
