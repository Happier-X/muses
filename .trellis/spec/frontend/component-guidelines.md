...
- **全 app 玻璃统一为官方系统玻璃（灰玻璃双层结构）**（08-12-unify-glass-effects）：导航/吸顶/播放条一律用双层——blur 层（`backdrop-blur-[2px]` + 需要渐显时加 `mask-b-from-50%`/`mask-b-to-100%`，无背景）+ bg 渐变层（`bg-gradient-to-b from-ios-light-surface to-transparent`，无 blur 无 mask），两层 absolute inset-0 分离 + pointer-events-none，内容层 relative。**禁止单层叠 mask**（mask 会把 blur 与渐变一起裁掉）。
