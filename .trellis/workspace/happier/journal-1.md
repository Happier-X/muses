...
播放控制按钮布局优化（顺序/随机、列表循环/单曲循环），播放响应速度优化（立即显示元数据、跳过重复提取、修复首次播放）
...
实现 PlayerBar 歌词显示：LRC 解析、WebDAV/本地歌词加载、ID3 嵌入歌词提取（USLT/TXXX 帧），实时同步播放进度
...
实现了点击PlayerBar弹出沉浸页面、下滑手势收起、修复了封面加载问题
...
## Session 9: Drawer refactor: push instead of overlay

**Date**: 2026-04-29
**Task**: Drawer refactor: push instead of overlay
**Branch**: `main`

### Summary

将左侧抽屉从 ModalNavigationDrawer 重构为自定义 Box 实现：抽屉改为半屏宽、无圆角，点击后内容通过 graphicsLayer translationX 向右推动，PlayerBar 固定在底部不受影响，支持菜单切换页面。
...
实现左滑切换歌词页面：新增 LyricsPage 组件（模糊封面背景、行级高亮、自动跟随滚动），改造 NowPlayingScreen 为 HorizontalPager 分页（封面/歌词），扩展 PlayerState 同步歌词索引，修复多处 bug（setValue import、autoScroll 竞争条件）
